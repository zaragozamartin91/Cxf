...
@WebServiceClient(name = "BookRepoService", wsdlLocation = "classpath:BookRepoImpl.wsdl", targetNamespace = "http://ws.ex.mz/")
public class BookRepoService extends Service {
	...
	static {
		URL url = BookRepo.class.getClassLoader().getResource("BookRepoImpl.wsdl");
		if (url == null) {
			java.util.logging.Logger.getLogger(BookRepo.class.getName()).log(java.util.logging.Level.INFO, "Can not initialize the default wsdl from {0}",
					"classpath:BookRepoImpl.wsdl");
		}
		WSDL_LOCATION = url;
	}
	...
	public BookRepo getBookRepoPort() {
		return super.getPort(BookRepoPort, BookRepo.class);
	}
	 ...
	 */
	@WebEndpoint(name = "BookRepoPort")
	public BookRepo getBookRepoPort(WebServiceFeature... features) {
		return super.getPort(BookRepoPort, BookRepo.class, features);
	}
